...
const defaultState = () => ({
  ...
  styles: {
    "style-01": { images: {}, created_at: null },
    "style-02": { images: {}, created_at: null }
  }
});

const normalizeState = (input) => {
  const fallback = defaultState();
  const styles = input && typeof input.styles === "object" && input.styles ? input.styles : fallback.styles;
...
const handleUpload = async (request, response) => {
  try {
    ...
    await updateState(async (state) => {
      ...
      state.styles[body.styleId].images ??= {};
      state.styles[body.styleId].images[body.slot] = publicPath;
      state.styles[body.styleId].metadata ??= {};
      state.styles[body.styleId].metadata[body.slot] = {
    ...
    sendJson(response, 201, { ok: true, path: publicPath });
  } catch (error) {
    sendJson(response, 400, { error: error.message || "上传失败" });
  }
};

const handleCreateStyle = async (response) => {
  try {
    const { state, result } = await updateState(async (draft) => {
      ...
      draft.styles[styleId] = { images: {}, created_at: new Date().toISOString() };
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/styles") return handleCreateStyle(response);
